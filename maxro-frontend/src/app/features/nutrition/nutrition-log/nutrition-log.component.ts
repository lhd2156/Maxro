            ...
            @if (activeTab === 'search') {
                      ...
                      <div class="result" [class.expanded]="expandedIdx === idx">
                        <div class="result-main" (click)="selectFoodCard(food, idx)">
                          ...
                          <div class="result-copy">
                            ...
                            @if (displayFoodContext()) {
                              <span class="result-helper">{{ displayFoodContext() }}</span>
                            }
              ...
              <form [formGroup]="customForm" (ngSubmit)="addCustomFood()" class="custom-form" [class.allow-scroll]="advancedOptionsOpen">
                ...
                <div class="custom-row">
                  ...
                  <mat-form-field appearance="outline"><mat-label>Unit</mat-label><input matInput formControlName="servingUnit"></mat-form-field>
...
export class NutritionLogComponent implements OnInit {
  ...
  get noResultsMessage(): string {
    ...
    return `No strong matches showed up for "${query}". Try the chain + item name, like "raising canes chicken fingers".`;
  }

  prevDay(): void { this.currentDate = new Date(this.currentDate.getTime() - 86400000); this.loadLog(); }
  nextDay(): void { this.currentDate = new Date(this.currentDate.getTime() + 86400000); this.loadLog(); }
  ...
  displayFoodContext(): string | null {
    const alias = this.searchBrandAlias;
    if (!alias) {
      return null;
    }
  ...
  private rebrandGenericRestaurantName(value: string, alias: string): string {
    ...
    if (/toast/.test(normalized)) return alias === "Raising Cane's" ? `${alias} Texas Toast` : `${alias} Toast`;
    if (/crinkle|fries/.test(normalized)) return alias === "Raising Cane's" ? `${alias} Crinkle-Cut Fries` : `${alias} Fries`;
